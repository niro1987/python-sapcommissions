...
def test_resource_basics(
    ...
    assert issubclass(resource_cls, model._Base), "resource is not a subclass of '_Base'"

    # _endpoint
    assert hasattr(resource_cls, "_endpoint"), "resource does not have attribute '_endpoint'"
    endpoint: ModelPrivateAttr = resource_cls.__private_attributes__["_endpoint"]
    assert isinstance(endpoint.default, str), "resource does not have a default value for '_endpoint'"
    assert endpoint.default.startswith("api/v2/"), "_endpoint should start with 'api/v2/'"

    # _attr_seq
    assert hasattr(resource_cls, "_attr_seq"), "resource does not have attribute '_attr_seq'"
    attr_seq: ModelPrivateAttr = resource_cls.__private_attributes__["_attr_seq"]
    assert isinstance(attr_seq.default, str), "resource does not have a default value for '_attr_seq'"
    ...
    assert seq_field.annotation == Union[str, None]
