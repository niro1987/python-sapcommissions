...
async def load_resource_filtered(
    ...
    resources: pd.DataFrame = pd.DataFrame()
    ...
    async for item in generator:
        ...
        if len(buffer) == MAX_BUFFER:
            ...
            resources = pd.concat([resources, chunk])
            buffer.clear()
    ...
    if resources.empty:
        raise SAPNotFoundError(resource_cls.__name__)

    return _transform_all(resources, resource_cls)
...
async def load_credits(  # pylint: disable=too-many-locals
    ...
    full_df['transaction'] = (
        full_df['sales_order_name']
        + ', Line:'
        + full_df['line_number'].astype(str)
        + ', Subline:'
        + full_df['sub_line_number'].astype(str)
        + ' '
        + full_df['event_type.event_type_id']
    )
    ...
    if filename:
        ...
        final_df: pd.DataFrame = (
            full_df[columns.keys()].rename(columns=columns).fillna('')
        )
